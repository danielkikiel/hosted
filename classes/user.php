...
class User {
    ...
    public function addInfoUser($fields = array()) 
    {
        $info = json_encode($fields, true);
        return $info;
    }
    public function infoUser($key) {
        $infou = $this->_db->get('users', array('id', '=', $this->data()->id));

        if($infou->count()) {
            $info = json_decode($infou->first()->info, true);
